...
int get_cone_colour(ConeColourCount_t cone_colour_count) {
    ...
    if (*max_iter == 0) {
        ...
        return custom_sys_msgs::msg::Cone::UNKNOWN;
    }

    int max_idx = std::distance(counts.begin(), max_iter);
    switch (max_idx) {
        case 0:
            return custom_sys_msgs::msg::Cone::BLUE;
        case 1:
            return custom_sys_msgs::msg::Cone::YELLOW;
        case 2:
            return custom_sys_msgs::msg::Cone::ORANGE_BIG;
        case 3:
            return custom_sys_msgs::msg::Cone::ORANGE_SMALL;
...
int initalise_new_landmark(Eigen::MatrixXd& mu, Eigen::MatrixXd& cov, double lm_map_x, double lm_map_y, double lm_range,
    ...
    int N = LANDMARK_STATE_SIZE;
    cov.bottomRows(N).setZero();
    cov.rightCols(N).setZero();
...
void compute_expected_z(const Eigen::MatrixXd& mu, int landmark_idx, Eigen::Matrix<double, 2, 1>& expected_z_out,
    ...
    // Downdated assignment using block operation
    int i = 0; // Starting row index for the block
    int j = landmark_idx; // Starting column index for the block
    int p = 2; // Number of rows in the block
    int cols = LANDMARK_STATE_SIZE; // Number of columns in the block

    jH_out.block(i, j, p, cols) << sqrt_q * dx, sqrt_q * dy,
                                -dy,        dx;
...
void EKFslam::update_pose(double x, double y, double theta) {
    // Update the robot's pose in the state vector using set_state_on_mu
    set_state_on_mu(mu, x, y, wrap_pi(theta)); // Ensure theta is within [-pi, pi)

    // You may need to adjust the covariance matrix accordingly.
    // This would depend on how the new pose affects your system's uncertainty.
}

void EKFslam::update_pose_and_velocity(double x, double y, double theta,
                                       const geometry_msgs::msg::Vector3& linear_velocity,
                                       const geometry_msgs::msg::Vector3& angular_velocity) {
    // Update the robot's pose in the state vector
    update_pose(x, y, theta);

    // Velocity updates can be included here if you later decide to 
    // incorporate velocity into your state vector.
}
void EKFslam::predict(double forward_vel, double rotational_vel, double dt, double uncertainty_time_weight,
    ...
    Eigen::Matrix3d uncertainty =
        motion_uncertainty(dt, theta, rotational_vel, forward_vel, uncertainty_time_weight, uncertainty_rotation_weight,
                          uncertainty_forward_weight, uncertainty_heading_time_weight);
    pred_cov.topLeftCorner(CAR_STATE_SIZE, CAR_STATE_SIZE) =
        jFx * pred_cov.topLeftCorner(CAR_STATE_SIZE, CAR_STATE_SIZE) * jFx.transpose() + uncertainty;
...
void EKFslam::update(const std::vector<custom_sys_msgs::msg::Cone>& detected_cones, double range_variance,
    ...
    for (auto const& cone : detected_cones) {
        if (cone.color == custom_sys_msgs::msg::Cone::ORANGE_BIG) {
            continue;
        }
...
void EKFslam::update_cone_colour(custom_sys_msgs::msg::Cone cone, int associated_landmark_idx) {
...
std::vector<custom_sys_msgs::msg::ConeWithCovariance> EKFslam::get_cones() {
    std::vector<custom_sys_msgs::msg::ConeWithCovariance> cones;
    for (uint i = 0; i < this->cone_colours.size(); i++) {
        int lm_idx = cone_idx_to_landmark_idx(i);
        custom_sys_msgs::msg::ConeWithCovariance cone;
